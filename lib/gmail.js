...
async function verifyEmailCredentials(config = getEmailConfig()) {
  if (!isGoogleApiConfigured(config)) {
    return {
      ok: false,
      configured: false,
      error: getEmailConfigError(),
    };
  }
  ...
  try {
    ...
    return { ok: true, configured: true, user: config.user };
  } catch (err) {
    const message = err.message || "Gmail OAuth verification failed.";
    const hint =
      message.includes("invalid_grant")
        ? "Refresh token expired or revoked. Run: npm run google:auth && npm run sync:gmail"
        : undefined;
...
function buildRawEmail({ from, to, bcc, subject, body, attachmentPath, attachmentFilename }) {
